...
{
    (*result).x = dot(make_float4(a[0][0], a[0][1], a[0][2], 0.f), b);
    (*result).y = dot(make_float4(a[1][0], a[1][1], a[1][2], 0.f), b);
    (*result).z = dot(make_float4(a[2][0], a[2][1], a[2][2], 0.f), b);
    (*result).w = 0.f;
}
...
void svd_solve_sym(mat3x3_tri a, float4* sigma, mat3x3 v) {
    ...
    *sigma = make_float4(vtav[0][0], vtav[1][1], vtav[2][2], 0.f);
}

float svd_invdet(float x, float tol) {
    return (fabs(x) < tol || fabs(1.0 / x) < tol) ? 0.0F : (1.0F / x);
}
...
extern "C" float qef_solve_d(float* AtA_tri, float* ATb, float* massPoint, float* x){
    mat3x3_tri ata = {};
    float4 Atb_;
    float4 ac;
    ...
    float er = qef_solve(ata, Atb_, ac, &ret);
